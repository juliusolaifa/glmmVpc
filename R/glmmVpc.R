singleGLMMFit <- function(formula, data, family, cov_values, nulltest=NULL) {
  ...
  beta <- tryCatch({
    unname(glmmTMB::fixef(modObj)$cond)
  }, error = function(e) {
    message("Error extracting fixed effects: ", e$message)
    return(NULL)
  })

  if (is.null(beta)) {
    return(NULL)  # Return NULL if fixed effects extraction failed
  }

  names(beta) <- paste0("b", 0:(length(beta)-1))
  Sigma <- tryCatch({
    unname(glmmTMB::VarCorr(modObj)$cond)[[1]]
  }, error = function(e) {
    message("Error extracting random effects: ", e$message)
    return(NULL)
  })

  if (is.null(Sigma)) {
    return(NULL)  # Return NULL if random effects extraction failed
  }

  phi <- tryCatch({
    glmmTMB::sigma(modObj)
  }, error = function(e) {
    message("Error extracting dispersion parameter: ", e$message)
    return(NULL)
  })
  names(phi) <- "phi"
  power <- tryCatch({
    unname(glmmTMB::family_params(modObj))
  }, error = function(e) {
    message("Error extracting family parameters: ", e$message)
    return(NULL)
  })
  ...
  if (is.null(vpcfunc)) {
    stop(paste("Function: ", vpcfunc_name, "is not defined"))
  }
